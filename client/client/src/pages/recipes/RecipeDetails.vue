<template>
    <section>
        <base-card>
            ...
            <base-button @click="addRecipeToList">Save the Recipe</base-button>
        </base-card>
        <section>
            <base-card>
                <h2>Ingredients:</h2>
...
export default {
    ...
    methods: {
        addRecipeToList() {
            if (this.listToAdd !== '') {
                this.$store.dispatch('recipes/addRecipe', {username: this.user.username, listId: this.listToAdd, recipe: this.recipe});
                this.$notify("This recipe has been added to the list!");
            }
        }
    },
    created() {
